/**
 * @file main.c
 * @brief Cxgen CLI tool.
 *
 * Usage: cxgen --yaml config.yaml --header Config.hpp --output config.gen.hpp
 *
...
static void print_usage(const char* prog) {
    fprintf(stderr, "Usage: %s --yaml <file> --header <file> --output <file> [--verbose]\n", prog);
    fprintf(stderr, "\n");
...
int main(int argc, char* argv[]) {
    ...
    if (!cg_parse_args(argc, argv, &args, &err)) {
    ...
    if (!cg_struct_parser_parse_file(parser, args.header_path, &err)) {
    ...
    if (args.verbose) {
        printf("Found %zu struct(s)\n", cg_struct_parser_get_struct_count(parser));
    ...
    cg_output* output = cg_generate(gen, args.yaml_path, args.header_path, &err);
    ...
    char* include_path = cg_path_relative_to_file(args.output_path, args.header_path);
    if (!include_path) {
        fprintf(stderr, "Error: Out of memory\n");
    ...
    fprintf(outfile, "%s", cg_output_get_code(output));
